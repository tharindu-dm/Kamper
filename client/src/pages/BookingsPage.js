    ...
    <div>
      ...
      <div className='mt-4'>
        {bookings?.length > 0 && bookings.map((booking) => (
          <div key={booking._id} className='flex mb-2 gap-4 bg-gray-200 rounded-2xl overflow-hidden p-3'>
            <div className="w-48">
              <PlaceImg place={booking.place} />
            </div>
            <div className="py-3 pr-3 grow">
              {/* Link only for the title or image, not the whole container */}
              <Link to={`/account/bookings/${booking._id}`} className="text-xl">
              ...
              <div className="text-xl">
                ...
                <div className="flex gap-2 mt-4">
                  {/* Update button */}
                  <button
                    onClick={() => navigate(`/account/bookings/update/${booking._id}`)}

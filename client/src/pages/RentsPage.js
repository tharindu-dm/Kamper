...
export default function RentingsPage() {
    ...
    const handleDelete = (rentingId) => {
        ...
        if (userConfirmed) {
            axios.delete(`/api/rentings/${rentingId}`)
                .then(() => {
                    setRentings(prerentings => prerentings.filter(renting => renting._id !== rentingId));
                })
                .catch(err => {
    ...
    return (
        <div>
            <AccountNav />
            <div className='mt-4'>
                {rentings.length > 0 && rentings.map((renting) => (
                    <div key={renting._id} className='flex mb-2 gap-4 bg-gray-200 rounded-2xl overflow-hidden p-3'>
                        <div className="w-48">
                        ...
                        <div className="py-3 pr-3 grow">
                            ...
                            <div className="text-xl">
                                <BookingDates booking={renting} className="mb-2 mt-4 text-gray-500" />
                  ...
                  {/* Update button */}
                  <button
                    onClick={() => navigate(`/account/rents/update/${renting._id}`)}

        ...
        <div>
            ...
            <div className='mt-4'>
                {rentings?.length > 0 && rentings.map((renting) => (
                    <Link key={renting._id} to={`/account/rents/${renting._id}`} className='flex mb-2 gap-4 bg-gray-200 rounded-2xl overflow-hidden p-3'>
                        <div className="w-48">
                        <GearImg gear={renting.gear} />
                        </div>
                        <div className="py-3 pr-3 grow">
                            <h2 className="text-xl">{renting.gear.title}</h2>
